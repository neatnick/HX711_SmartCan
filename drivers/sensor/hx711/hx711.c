 ...
 *
 */
void hx711_attr_set_offset(struct hx711_data *data, const struct sensor_value *offset)
{
 ...
 *
 */
void hx711_attr_set_slope(struct hx711_data *data, const struct sensor_value *slope)
{
...
int avia_hx711_tare(const struct device *dev, uint8_t readings)
{
	...
	for (int i = 0; i < readings; i++)
	{
		...
		{
			printk("\n throw away : ");
			printk("Tare reading = %d", data->reading);
		}
		else
		{
			avg += data->reading;
			printk("\n Tare reading = %d", data->reading);
		}
	}
	LOG_DBG("Average before division : %d", avg);
	// avg = avg / (readings);
	avg = avg / (readings - throwaway_val);
										 ...
										 uint8_t readings)
{
	...
	for (int i = 0; i < readings; i++)
	{
		...
		{
			printk("\n throw away : ");
			printk("Cal reading = %d", data->reading);
		}
		else
		{
			avg += data->reading;
			printk("\n Cal reading = %d", data->reading);
		}
	}
	LOG_DBG("Average before division : %d", avg);
	// avg = avg / readings;
